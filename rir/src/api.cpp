...
static pir::TypeSignature parseTypeSignature(SEXP sigSexp) {
    // TODO: Valid format checks
    if (sigSexp == R_NilValue)
        return pir::TypeSignature::any();
    if (TYPEOF(sigSexp) != STRSXP)
        Rf_error("can't parse pir signature - must be a string");
    return pir::TypeSignature::parse(CHAR(Rf_asChar(sigSexp)));
}

REXPORT SEXP rir_mkSignature(
...
#define V(n)                                                                   \
    if (Rf_asLogical(n##Sexp))                                                 \
        signature.set(pir::ClosureSignatureFlag::n);
    LIST_OF_CLOSURE_SIGNATURE_FLAGS(V)
#undef V

    signature.type = parseTypeSignature(sigSexp);
    SEXP res = Rf_allocVector(RAWSXP, sizeof(pir::ClosureSignature));
    memcpy(DATAPTR(res), (void*)&signature, sizeof(pir::ClosureSignature));
    return res;
}
