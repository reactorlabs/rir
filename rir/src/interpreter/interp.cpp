...
#undef DEF_INSTR
    ...
    BEGIN_MACHINE {
        ...
        INSTRUCTION(vapply_) {
            SEXP R_fcall, ans, names = R_NilValue, rowNames = R_NilValue, X, XX,
                               FUN, value, dim_v;

            X = cp_pool_at(ctx, readImmediate());
            advanceImmediate();
            R_xlen_t i, n;
            int commonLen;
            int useNames,
            ...
            XX = ostack_at(ctx, 3);
            FUN = ostack_at(ctx, 2);
            value = ostack_at(ctx, 1);
            if (!isVector(value))
                Rf_error(("'FUN.VALUE' must be a vector"));
            useNames = asLogical(ostack_at(ctx, 0));
            ...
            if (commonLen > 1 && n > INT_MAX)
                Rf_error((
                    "long vectors are not supported for matrix/array results"));
            ...
            if (useNames) {
                ...
                PROTECT_WITH_INDEX(
                    rowNames = getAttrib(value, array_value ? R_DimNamesSymbol
                                                            : R_NamesSymbol),
                    &index);
            }
            ...
            */
            {
                ...
                PROTECT(R_fcall = LCONS(
                            FUN, LCONS(tmp, LCONS(R_DotsSymbol, R_NilValue))));
                ...
                for (i = 0; i < n; i++) {
                    ...
                    if (valType != commonType) {
                        ...
                        switch (commonType) {
                        case CPLXSXP:
                            okay = (valType == REALSXP) ||
                                   (valType == INTSXP) || (valType == LGLSXP);
                            break;
                        case REALSXP:
                    ...
                    if (commonLen == 1) { // common case
                        ...
                        case INTSXP:
                            memcpy(INTEGER(ans) + common_len_offset,
                                   INTEGER(val), commonLen * sizeof(int));
                            break;
                        case LGLSXP:
                            memcpy(LOGICAL(ans) + common_len_offset,
                                   LOGICAL(val), commonLen * sizeof(int));
                            break;
                        case RAWSXP:
                        ...
                        case CPLXSXP:
                            memcpy(COMPLEX(ans) + common_len_offset,
                                   COMPLEX(val), commonLen * sizeof(Rcomplex));
                            break;
                        case STRSXP:
            ...
            UNPROTECT(
                useNames
                    ? 3
                    : 1); /* X, XX, value, ans, and maybe names and rowNames */
            ostack_popn(ctx, 4);
            ostack_push(ctx, ans);
