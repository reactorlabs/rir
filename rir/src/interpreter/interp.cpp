...
#undef DEF_INSTR
    ...
    BEGIN_MACHINE {
        ...
        INSTRUCTION(call_eager_) {
            ...
            SEXP newEnv = PROTECT(Rf_NewEnvironment(R_NilValue, R_NilValue, CLOENV(callee)));

            SEXP f = FORMALS(callee);
            for (size_t i = 0; i < n; ++i) {
                SEXP arg = ostack_at(ctx, n - i - 1);
                Rf_defineVar(TAG(f), arg, newEnv);
                f = CDR(f);
            }
            if (R_envHasNoSpecialSymbols(newEnv))
                SET_NO_SPECIAL_SYMBOLS(newEnv);
            ...
            if (R_GlobalContext->callflag == CTXT_GENERIC)
                Rf_begincontext(&cntxt, CTXT_RETURN, call, newEnv,
                                R_GlobalContext->sysparent, R_NilValue, callee);
            else
                Rf_begincontext(&cntxt, CTXT_RETURN, call, newEnv, env, R_NilValue,
                                callee);
