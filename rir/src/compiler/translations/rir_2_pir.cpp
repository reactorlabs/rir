...
pir::Function* Rir2Pir::compileFunction(SEXP function2Compile){
    ...
    rir::Function* rirFunction = tbl->first();
    return this->compileFunction(rirFunction);
}
...
pir::Function* Rir2Pir::compileFunction(IRTransformation* rir2PirTransformation, bool addReturn) {
    ...
    pir::Function* pirFunction = rir2PirTransformation->dstIR;
    ...
    while (state.getPC() != end || !worklist.empty()) {
        ...
        if (bc.isJmp()) {
            ...
            case Opcode::brobj_: {
                ...
                builder(new Deopt(builder.env, state.getPC(), state.stack_size(), &front));
                break;
            }
            default:
                assert(false);
...
Module* Rir2Pir::compileModule(SEXP f, bool verbose) {
    for (auto f : this->module->functions) {
        this->compileFunction(f, verbose);
    }
    return this->module;
}   

void Rir2Pir::operator()(SEXP in) {
