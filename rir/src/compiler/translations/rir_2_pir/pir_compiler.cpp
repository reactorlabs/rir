...
RirInput PirCompiler::createRirInputFromSEXP(SEXP closure, std::vector<SEXP>& fmls, Env* env) {
...
IRCode PirCompiler::compile(IRCode sourceIR){
    ...
    Closure* pirFunction = module->getOrCreate(rirSource->function, *rirSource->args, rirSource->env, 
        [&](Closure* pirFunction) {});
    Builder builder(pirFunction, rirSource->env);
    this->setBuilder(&builder);
    if (isVerbose()) {
        std::cout << " ========== Compiling Rir Function: " << rirSource->function << "\n";
    }
...
void PirCompiler::applyOptimizations(Closure* f,
