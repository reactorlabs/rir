...
bool InlineForcePromises::apply(Compiler&, ClosureVersion* cls, Code* code,
                ...
                if (c->kind == CastType::Upcast) {
                    if (auto mk = MkArg::Cast(c->arg(0).val())) {
                        if (mk->isEager() && mk->prom()->trivial()) {
                            // these are their own ast-expression, so substitute
    and
    ...
    { if (!mk->isEager()) { if (!isHuge || mk->prom()->size() < 10) {
                                        // We need to know if the promise
    escaped
                ...
                if (auto f = Force::Cast(*ip)) {
                    if (auto mkarg = MkArg::Cast(f->followCastsAndForce())) {
                        if (mkarg->isEager()) {
                            ...
                            BB* split =
                                BBTransform::split(code->nextBBId++, bb, ip,
    code);

                            // Note that the entry block is empty and jumps to
    the
                            ...
                            assert(entry->isEmpty() && entry->isJmp() &&
                                   !entry->next()->isEmpty());
                            BB* prom_copy =
                                BBTransform::clone(prom->entry->next(), code,
    cls); bb->overrideNext(prom_copy);
                                ...
                                while (it != bb->end()) {
                                    ...
                                    if (f->frameState()) {
                                        if (auto sp = FrameState::Cast(*it)) {
                                            if (!sp->next()) {
                                                auto copyFromFs =
    f->frameState(); auto cloneSp = FrameState::Cast( copyFromFs->clone());
                                                ...
                                                while (copyFromFs->next()) {
                                                    assert(copyFromFs->next() ==
                                                           cloneSp->next());
                                                    copyFromFs =
    copyFromFs->next(); auto prevClone = cloneSp; cloneSp = FrameState::Cast(
                                                        copyFromFs->clone());
    ...
    ++it) { if (LdFunctionEnv::Cast(*it)) { assert(!promenvBB); promenv = it;
                                        promenvBB = (*it)->bb();
                                    }
                                }
                            });
    ...
    { if (auto fi = Force::Cast(i)) { if (fi->hasEnv()) { fi->env(f->env());
                                            }
                                        }
                                    });
                                }
    ...
    f->env()); auto promRes = promRet.first;
                            ...
                            MkArg* fixedMkArg =
                                new MkArg(mkarg->prom(), promRes,
    mkarg->promEnv()); pos = split->insert(pos, fixedMkArg); pos++; CastType*
    upcast = new CastType( fixedMkArg, CastType::Upcast, RType::prom,
                            ...
                            if (u.kind == ForcedBy::PromiseInlineable::
                                ...
                                if (auto m = MkEnv::Cast(u.escapedAt())) {
                                    m->eachLocalVar([&](SEXP name, Value* a,
    bool) { if (a->followCasts() == mkarg) { pos = split->insert( pos, new
    StArg(name, upcast, m)); pos++;
                                        }
                                    });
                                    ...
                                    assert(false && "Cannot deal with this
    leak");
                                }
                            }
                            next = pos;
                                ...
                                auto allowedToReplace = [&](Instruction* i) {
                                    ...
                                    if (builtinId) {
                                        if (eager->type.maybeObj())
                                            if
    (SafeBuiltinsList::always(builtinId)) return true; if
    (SafeBuiltinsList::nonObject(builtinId)) return true;
                                    }
                                    if (i->effects.includes(Effect::LeakArg) ||
                                        i->effects.includes(Effect::Reflection))
    { return false;
                                    }
                                    // Depromised and promised missing do not
    behave
                                    // the same (promised missing does not cause
                                    // default arguments to be used)! So let's
    only
                ...
                if (auto c = CastType::Cast(i)) {
                    if (auto m = MkArg::Cast(c->arg(0).val())) {
                        ...
                        if (r != forcedMkArg.end()) {
                            ...
                            SLOWASSERT(c->usesAreOnly(repl->bb(),
    {Tag::MkEnv}));
                        }
                    }
                }
