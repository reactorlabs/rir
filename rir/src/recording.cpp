...
SEXP CompilationEvent::toSEXP() const {
    ...
    auto speculative_contexts_sexp =
        Rf_allocVector(VECSXP, this->speculative_contexts.size());
...
void CompilationEvent::fromSEXP(SEXP sexp) {
    ...
    this->compile_reasons.heuristic =
        serialization::compile_reason_from_sexp(VECTOR_ELT(sexp, 4));
    this->compile_reasons.condition =
        serialization::compile_reason_from_sexp(VECTOR_ELT(sexp, 5));
    this->compile_reasons.osr =
        serialization::compile_reason_from_sexp(VECTOR_ELT(sexp, 6));
}
