...
class Compiler {
  ...
  public:
    ...
    static void tryLinking(DispatchTable * vtab, SEXP hSym) {
        ...
        SEXP bcData = Rf_findVarInFrame(serMap, hSym);
        if (bcData != R_UnboundValue && bcData != R_NilValue) {
            // iterate over bitcodes

            serializerData::iterateOverOffsets(bcData, [&] (SEXP offsetSymbol, SEXP offsetEnv) {
