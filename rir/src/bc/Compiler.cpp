    ...
    compileWhile(
        ...
        [&ctx, &cs, &sym, &body]() {
            ...
            if (ctx.code.top()->isCached(sym))
                ...
                cs << BC::stvar(sym);
            ctx.cfgBuilder.markParameterExcluded(sym);
            // i' <- i' + step
        ...
        if (fun2 == symbol::Bracket || fun2 == symbol::DoubleBracket) {
            ...
            if (superAssign) {
                ...
                if (ctx.code.top()->isCached(target))
                    ...
                    cs << BC::stvar(target);
                ctx.cfgBuilder.markParameterExcluded(target);
            }
            ...
            */
            ...
            if (superAssign) {
                ...
                if (ctx.code.top()->isCached(dest))
                    ...
                    cs << BC::stvar(dest);
                ctx.cfgBuilder.markParameterExcluded(dest);
            }
...
void compileGetvar(CompilerContext& ctx, SEXP name) {
    ...
    if (DDVAL(name)) {
        ...
        if (Compiler::profile) {
            cs << ctx.recordTypeAndTrack(name);
        }
    }
}
