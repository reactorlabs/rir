...
# TODO: FIXXXXX
stopifnot(
  pir.check(mandelbrot, NoExternalCalls, NoPromise, warmup = function(f) {
    f(13)
    f(27)
  })
)
...
stopifnot(
  pir.check(function(arg) {
    nbodyPrologue(arg)
  }, NoExternalCalls, NoPromise, NoEnvSpec, warmup = function(f) {
    f(10)
    f(10)
  })
)
...
stopifnot(
  pir.check(simplifiedBounceInit, NoEnvSpec, NoPromise, warmup = function(f) {
    f()
  })
)
...
stopifnot(pir.check(emptyFor, OneAdd, AnAddIsNotNAOrNaN, warmup = function(f) {
  f(1000)
}))
arg <- 1000
stopifnot(pir.check(emptyFor, OneAdd, AnAddIsNotNAOrNaN, warmup = function(f) {
...
stopifnot(pir.check(f, NoExternalCalls, warmup = function(f) {
  f()
  f()
}))
...
stopifnot(pir.check(h, NoExternalCalls, Returns42L, warmup = function(h) {
  h()
  h()
}))
