...
function build_r {
    ...
    if [ -f "${SRC_DIR}/rsync-recommended" ]; then
        echo "Found modified rsync-recommended, using it"

        cp ${SRC_DIR}/rsync-recommended ${R_DIR}/tools
        echo "copying ${SRC_DIR}/rsync-recommended --> ${R_DIR}/tools"

        if [ -d "${SRC_DIR}/recommended" ]; then
            cp ${SRC_DIR}/recommended/* ${R_DIR}/src/library/Recommended
            echo "copying ${SRC_DIR}/recommended/* --> ${R_DIR}/src/library/Recommended"
        else
            echo "ERROR, unable to find: ${SRC_DIR}/recommended"
            exit 1
        fi
    fi
