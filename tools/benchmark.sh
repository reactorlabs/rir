...
for i in `ls ${BASE}/benchmarks/shootout/*/*.r`; do
    ...
    timeout $TIMEOUT /usr/bin/time -f'%E' -- ${SCRIPTPATH}/R -e "{rir.enableJit(level=3, type='sticky');setwd('$D'); source('$i'); execute()}" > /dev/null;
    timeout $TIMEOUT /usr/bin/time -f'%E' -- ${SCRIPTPATH}/R -e "{rir.enableJit(level=3, type='force');setwd('$D'); source('$i'); execute()}" > /dev/null;
done 
