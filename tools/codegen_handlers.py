...
class Manager:
    ...
    def getTypeFromXML(self, xml):
        ...
        # or it is a known type in which case we load it through the manager
            ...
            if (len(xml) == 0):
                # it's a constructor or destructor, we are not interested in these
                return None
            else:
                xml = xml[0]
                if (xml.tag != "ref"):
...
class Handler:
    ...
    class DispatchTable:
        ...
        class Entry:
            ...
            def __str__(self):
                result = "{0}: {1} or ".format(self.type, self.unconditional.name if self.unconditional else "nodefault")
            ...
            def emitUnconditional(self, iterators):
                if (self.unconditional):
                    result = self.unconditional.name + self.emitUnconditionalCall(iterators)
                    return result
...
""".format(matched = iterators[-2], dispatch = self.recursive.emit(iterators[:]).replace("\n", "\n        "))
                ...
                for h in self.conditional:
                    result += "    {0}\n".format(self.emitConditional(iterators, h))

                # then go for the single non-conditional one
                if (self.unconditional):
                    result += "    {0};\n".format(self.emitUnconditional(iterators))
                    # assign the last iterator to the first one (first one is ref arg)
                    result += "    {first} = {last};\n    return true;\n".format(first = iterators[0], last = iterators[-1])
        ...
        def emit(self, incomingIterators):
            ...
            """
            # last iterator is the iterator on which we should call the match function to get the type of instruction to dispatch on
            lastIterator = incomingIterators[-1]
            it = lastIterator + lastIterator[0];
            # add the last iterator to incoming iterators so that entry's emits will find it
...
bool dispatch(llvm::BasicBlock::iterator & i) {{
    ...
    {parents}
    return false;
}}""".format(code = self._table.emit(["i"]).replace("\n", "\n    "), parents = parents)
...
def analyzeMatchSets(c):
    ...
    if (not c.subclasses):
        ...
        c.matchSet = m
        return m
    # otherwise the matchset of the class is union of all match sets of its children
...
# first load the class we need, that is 
print("initializing...")
m = Manager("c:/delete/xml")
...
#x = et.parse("c:/delete/xml/classrjit_1_1ir_1_1_instruction.xml")
#y = x.getroot()
#print(y.attrib)
#print(y.tag)
#for child in y[0]:
