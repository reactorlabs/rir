...
private:
    ...
    llvm::Value * compileICCallStub(llvm::Value * call, llvm::Value * op, std::vector<llvm::Value*> & callArgs);
    ...
    /** Compiles operators that can be either binary, or unary, based on the number of call arguments. Takes the binary and unary intrinsics to be used and the full call ast.
      */
    llvm::Value * compileBinaryOrUnary(llvm::Function * b, llvm::Function * u, SEXP call);
