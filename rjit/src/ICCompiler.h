...
private:
     ...
     */
    llvm::Value * compileArgument(llvm::Value * arglist, SEXP argAst, int argnum, bool eager);
    ...
    llvm::Value * INTRINSIC_NO_SAFEPOINT(llvm::Value * fun, Values... args) {
        return INTRINSIC_NO_SAFEPOINT(fun, std::vector<llvm::Value*>({args...}));
    }

    llvm::Value * INTRINSIC_NO_SAFEPOINT(llvm::Value * fun, std::vector<llvm::Value*> args);
