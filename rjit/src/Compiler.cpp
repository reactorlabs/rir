...
Compiler::Context::Context(std::string const & name, llvm::Module * m) {
    f = llvm::Function::Create(t::sexp_sexpsexpint, llvm::Function::ExternalLinkage, name, m);
    setupFunction(*f);
...
SEXP Compiler::compileFunction(std::string const & name, SEXP ast, bool isPromise) {
    Context * old = context;
