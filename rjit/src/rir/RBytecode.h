...
/** TODO We basically want the functionality of mkPROMSXP, line 2333 memory.c.
  ...
  As a hack I am just using one of the old rjit bytecodes which did exactly the
  same.
*/
...
class RBytecode {
  public:
    /** Function bytecode version. Contains serialized bytecodes for the
     * function and all its promises (recursively), as well as the ast maps.
     */
    static constexpr int MAGIC_VERSION = 0xff;

    RBytecode(SEXP from) : data_(from) {
        assert(TYPEOF(from) == BCODESXP and
               "Creating RBytecode from something that is not bytecode");
        assert(version() >= MAGIC_VERSION and
               "Invalid version - perhaps not rir bytecode");
    }
     ...
     */
    operator SEXP() { return data_; }

    Code::CC callingConvention() const { return header_().cc; }

    /** Returns the version of the bytecode.
     */
    int version() const {
        // not using the header so that we do not assume anything about the
        // underlying sexp
     ...
     */
    SEXP ast() const { return VECTOR_ELT(consts_(), header_().nchildren); }

    /** Returns pointer to the rir bytecode contained in the object.
     */
    BC_t* bytecode() const {
        return reinterpret_cast<BC_t*>(INTEGER(code_()) +
                                       sizeof(Header) / sizeof(int));
    }

    size_t bytecodeSize() const { return header_().codeSize; }

    size_t astMapSize() const {
        return INTEGER(code_())[sizeof(Header) / sizeof(int) +
                                toIntSize(header_().codeSize)];
    }

    unsigned* astMapPos() const {
        return reinterpret_cast<unsigned*>(
            INTEGER(code_())[sizeof(Header) / sizeof(int) +
                             toIntSize(header_().codeSize) + 1]);
    }
    ...
    static RBytecode serialize(Code* from, Code::CC cc) {
        ...
        size_t codeSize = sizeof(Header) / sizeof(int) + toIntSize(from->size) +
                          1 + from->astMap.size;
        ...
        INTEGER(code)
        [sizeof(Header) / sizeof(int) + toIntSize(from->size)] =
            from->astMap.size;
        ++idx;
        // store the astmap positions
        memcpy(INTEGER(code) + idx, from->astMap.pos,
               from->astMap.size * sizeof(int));
        ...
        for (idx = 0; idx < nchildren; ++idx)
            SET_VECTOR_ELT(consts, idx,
                           serialize(from->children[idx], Code::CC::envLazy));
    ...
    Code* deserialize() {
        ...
        for (size_t i = 0; i < nchildren; ++i)
            result->addCode(i,
                            RBytecode(VECTOR_ELT(consts_(), i)).deserialize());
  ...
  private:
    static int toIntSize(int offset) {
        static_assert(
            sizeof(int) == 4,
            "Integer is four bytes long and R's integers are integers!");
        if (offset % 4 != 0)
            offset += 4 - (offset % 4);
        return offset / 4;
    }
    ...
    struct Header {
        ...
        Header(Code::CC cc, int nchildren, int codeSize)
            : version(MAGIC_VERSION), cc(cc), nchildren(nchildren),
              codeSize(codeSize) {}
    };

    static_assert(sizeof(Header) == 4 * sizeof(int),
                  "Header should not have any padding");
    static_assert(sizeof(Header) % sizeof(BC_t) == 0,
                  "To make sure alignment won't cause us trouble");

    RBytecode(SEXP code, SEXP asts) {
        // note that R_bcEncode is not required as threaded code makes no
        // difference to us
    ...
    SEXP code_() const { return BCODE_CODE(data_); }

    SEXP consts_() const { return BCODE_CONSTS(data_); }
...
class RFunction {
  public:
    RFunction(SEXP from) : data_(from) {
        assert(TYPEOF(from) == CLOSXP and
               "Function can only be created from Closure SEXPs");
    }
  ...
  private:
    SEXP body_() const { return BODY(data_); }

    SEXP formals_() const { return FORMALS(data_); }
...
static_assert(sizeof(RBytecode) == sizeof(SEXP),
              "RBytecode is assumed to be only a wrapper");
static_assert(sizeof(RFunction) == sizeof(SEXP),
              "RFunction is assumed to be only a wrapper");
static_assert(sizeof(RPromise) == sizeof(SEXP),
              "RPromise is assumed to be only a wrapper");
}
}
