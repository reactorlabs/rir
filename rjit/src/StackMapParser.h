  ...
  public:
    ...
    class LocationAccessor {
      ...
      public:
        ...
        uint32_t getSmallConstant() const {
            assert(getKind() == LocationKind::Constant &&
                   "Not a small constant.");
            return read<uint32_t>(P + SmallConstantOffset);
        }

        /// Get the constant-index for this location. (Kind must be
        /// ConstantIndex).
      ...
      private:
        ...
        static const int SmallConstantOffset =
            DwarfRegNumOffset + sizeof(uint16_t);
    ...
    class RecordAccessor {
      ...
      public:
        ...
        /// Get the instruction offset (from the start of the containing
        /// function)
        /// for this record.
    ...
    StackMapV1Parser(ArrayRef<uint8_t> StackMapSection)
        ...
        ConstantsListOffset =
            FunctionListOffset + getNumFunctions() * FunctionSize;
    ...
    record_iterator records_end() const {
        // Records need to be handled specially, since we cache the start
        // addresses
        // for them: We can't just compute the 1-past-the-end address, we have
        // to
