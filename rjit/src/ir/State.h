...
class AState {
  public:
    ...
    AVALUE& operator[](ir::Value index) { return registers_[index]; }

    AVALUE& operator[](SEXP symbol) { return variables_[symbol]; }

    AState& operator=(AState<AVALUE> const&) = default;

    /** Merges the other state into itself. Returns true if the state changed
     * during the merge.
     */
    ...
    void replaceWith(ir::Value old, ir::Value with) {
        assert(registers_.find(with) == registers_.end() and
               "Target value is assumed to be new");
  ...
  protected:
    ...
    bool mergeMaps(MAP& myMap, MAP const& theirMap) {
        ...
        // we do not care about any values in ourselves that are not part of
        // incomming as they will merge to themselves
        return result;
    }
