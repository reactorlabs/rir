...
class ForwardDriver : public PassDriver<PASS> {
  ...
  public:
    ...
    PASS* pass() { return &pass_; }

  protected:
    /** Executes the analysis pass on given block, checking whether a fixpoint
     * has been reached, updating the current state and enqueuing its
     * successors.
     */
    void runOnBlock(llvm::BasicBlock* block, typename PASS::State&& incomming) {
        ...
        for (unsigned ii = 1, end = t->getNumSuccessors(); ii < end; ++ii)
            q_.push_back(std::pair<llvm::BasicBlock*, typename PASS::State>(
                t->getSuccessor(ii), pass_.state));
        // enque first successor with move semantics
        if (t->getNumSuccessors() > 0)
            q_.push_back(std::pair<llvm::BasicBlock*, typename PASS::State>(
                t->getSuccessor(0), std::move(pass_.state)));
    }
...
/** A driver for optimizations.
  ...
  Is templated by the optimization's pass class and by the types of analyses
  that are required by the pass.

  Linearly passes over the basic blocks in the function and advances all
  required analyses as well.

  TODO this only works for forward analyses, we might beed a backward
  optimization pass too, or a backward analysis advancement that will work with
  fw passes too.
...
class OptimizationDriver : public ir::PassDriver<PASS> {
  public:
    ...
    /** LLVM's runOnFunction interface.
      ...
      The function will only be analysed if it is not a declaration and is not
      empty.
     */
  ...
  protected:
    ...
    virtual bool optimize(llvm::Function* f) {
        setFunction(f);
        for (auto& b : *f) {
            // at the beginning of each basic block we must set the proper state
            // to all analyses we require
            if (setState<ANALYSES...>(&b)) {
                llvm::BasicBlock::iterator i = b.begin();
                while (i != b.end()) {
                    // each analysis must advance its state past the instruction
                    // TODO the fact we do this before the optimization is fine
                    // because the state before cannot be different for the
                    // inputs of currently used instructions & patterns
    ...
    virtual void setFunction(llvm::Function* f) { pass.setFunction(f); }
  ...
  private:
    ...
    template <typename T1, typename T2>
    bool setState(llvm::BasicBlock* bb) {
        return this->template getAnalysis<T1>().pass()->setState(bb) and
