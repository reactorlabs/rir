...
No need for all the other things here because we do not support other than
generic variable loads and stores.
*/
...
/** Given the for loop sequence, and index, returns the index-th value of the
sequence.
...
/** When LLVM IR creates user visible constant, this function contains all the
code required to make the constant.
Currently this means taking the value from the constant pool and marking it as
not mutable.
*/
...
TODO this means we can get rid of the checks in getvar(), and reduce its code to
this. We definitely want faster versions.
*/
...
/** Creates a promise out of the given code and environment and returns its
 * value.
*/
...
We can perfectly do this in LLVM, but having an function for it simplifies the
analysis on our end.
*/
