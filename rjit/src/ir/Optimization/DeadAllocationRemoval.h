...
/** Removes unnecessary allocation for scalars.
  ...
  Scalars use the CreateAndSetScalar pattern to allocate a scalar SEXP and set its value from the unboxed one. Because allocation is a call to C function, LLVM would not understand it as dead code so this simple pass deletes all scalar alocations that are only used to set the value, but never afterwards.
 */
