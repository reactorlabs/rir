/*
 * RJit info: this is 1:1 copy from llvm 3.7 with NoEntry and NoBackedge enabled
 * to disable all safepoint poll insertion
 */
...
static cl::opt<bool> NoEntry("spp-no-entry", cl::Hidden, cl::init(true));
static cl::opt<bool> NoCall("spp-no-call", cl::Hidden, cl::init(false));
static cl::opt<bool> NoBackedge("spp-no-backedge", cl::Hidden, cl::init(true));
    ...
    while (true) {
        for (Instruction& I : *Current) {
            if (auto CS = CallSite(&I))
                ...
                // method
                // has an
                // unconditional poll. In practice, this is only a theoretical
                // concern
...
static bool mustBeFiniteCountedLoop(Loop* L, ScalarEvolution* SE,
    ...
    // being outside the loop, we can ask questions about the execution
    // frequency
    // of the exit block.
    if (L->isLoopExiting(Pred)) {
        ...
        if (MaxExec != SE->getCouldNotCompute()) {
            if (SE->getUnsignedRange(MaxExec).getUnsignedMax().ult(
                    upperTripBound))
                return true;
            if (SkipCounted &&
...
bool PlaceBackedgeSafepointsImpl::runOnLoop(Loop* L) {
    ...
    // having run sometime earlier in the pipeline, but this code must be
    // correct
    ...
    for (BasicBlock* pred : LoopLatches) {
        ...
        if (!AllBackedges) {
            ...
            if (CallSafepointsEnabled &&
                ...
                // Note: This is only semantically legal since we won't do any
                // further
                // IPO or inlining before the actual call insertion..  If we
                // hadn't, we
        ...
        // not help runtime performance that much, but it might help our ability
        // to
...
static bool doesNotRequireEntrySafepointBefore(const CallSite& CS) {
    ...
    if (IntrinsicInst* II = dyn_cast<IntrinsicInst>(Inst)) {
        ...
        case Intrinsic::experimental_patchpoint_i64:
            // The can wrap an actual call which may grow the stack by an
            // unbounded
            // amount or run forever.
            return false;
        default:
            // Most LLVM intrinsics are things which do not expand to actual
            // calls, or
            // at least if they do, are leaf functions that cause only finite
            // stack
            // growth.  In particular, the optimizer likes to form things like
            // memsets
            // out of stores in the original IR.  Another important example is
            // llvm.localescape which must occur in the entry block.  Inserting
            // a
            // safepoint before it is not legal since it could push the
            // localescape
...
static Instruction* findLocationForEntrySafepoint(Function& F,
    ...
    for (cursor = F.getEntryBlock().begin(); hasNextInstruction(cursor);
        ...
        // easiest way to ensure finite execution between safepoints in the face
        // of
        // recursive and mutually recursive functions is to enforce that each
        // take
        ...
        // which detect stack overflow via guard pages and then throw
        // exceptions.
...
bool PlaceSafepoints::runOnFunction(Function& F) {
    ...
    if (enableBackedgeSafepoints(F)) {
        ...
        // We can sometimes end up with duplicate poll locations.  This happens
        // if
        // a single loop is visited more than once.   The fact this happens
        // seems
        // wrong, but it does happen for the split-backedge.ll test case.
        PollLocations.erase(
            std::unique(PollLocations.begin(), PollLocations.end()),
            PollLocations.end());
        ...
        for (TerminatorInst* Term : PollLocations) {
            ...
            if (SplitBackedge) {
                // Split the backedge of the loop and insert the poll within
                // that new
                // basic block.  This creates a loop with two latches per
                // original
                // latch (which is non-ideal), but this appears to be easier to
                // optimize in practice than inserting the poll immediately
                // before the
                // latch test.

                // Since this is a latch, at least one of the successors must
                // dominate
                // it. Its possible that we have a) duplicate edges to the same
                // header
                // and b) edges to distinct loop headers.  We need to insert
                // pools on
                // each.
                ...
                // The split loop structure here is so that we only need to
                // recalculate
                // the dominator tree once.  Alternatively, we could just keep
                // it up to
                // date and use a more natural merged loop.
                SetVector<BasicBlock*> SplitBackedges;
                for (BasicBlock* Header : Headers) {
                    BasicBlock* NewBB =
                        SplitEdge(Term->getParent(), Header, &DT);
    ...
    if (enableCallSafepoints(F)) {
        ...
        for (auto c : Calls) {
            ...
            Attribute attr_id = attrs.getAttribute(AttributeSet::FunctionIndex,
                                                   "statepoint-id");
    ...
    for (size_t i = 0; i < ParsePointNeeded.size(); i++) {
        ...
        // destination block.
        // Reason for this is that we can place gc_result only after last phi
        // node
...
InsertSafepointPoll(Instruction* InsertBefore,
    ...
    // Include only the newly inserted instructions, Note: begin may not be
    // valid
...
static Value* ReplaceWithStatepoint(const CallSite& CS, /* to replace */
    ...
    if (CS.isCall()) {
        ...
        Builder.SetCurrentDebugLocation(
            ToReplace->getNextNode()->getDebugLoc());
    } else if (CS.isInvoke()) {
