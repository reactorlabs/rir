...
RUN cd /opt/rir && tools/sync.sh && tools/build-libjit.sh && tools/build-gnur.sh custom-r && rm -rf external/custom-r/cache_recommended.tar .git && find external -type f -name '*.o' -exec rm -f {} \; &&\
    ...
    rm -rf /opt/rir/external/libjit /opt/rir/external/llvm-8.0.0 /opt/rir/external/*.tar.xz /opt/rir/build/*/CMakeFiles /opt/rir/external/custom-r/src/main
