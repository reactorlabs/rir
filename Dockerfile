...
RUN echo $CI_COMMIT_SHA > /opt/rir_version && \
    apt-get update && \
    DEBIAN_FRONTEND=noninteractive apt-get upgrade -o dir::cache::archives="/apt-cache" -y -qq && \
    DEBIAN_FRONTEND=noninteractive apt-get install -o dir::cache::archives="/apt-cache" -y -qq curl git gcc gfortran g++ libreadline-dev libx11-dev libxt-dev zlib1g-dev libbz2-dev liblzma-dev libpcre3-dev libcurl4-openssl-dev libcairo2-dev make libreadline8 && \
    cd /opt/rir && \
    tools/build-gnur.sh && \
...
RUN mkdir -p /opt/rir/build/release && \
    DEBIAN_FRONTEND=noninteractive apt-get install -o dir::cache::archives="/apt-cache" -y -qq xz-utils cmake libtinfo5 && \
    cd /opt/rir && \
