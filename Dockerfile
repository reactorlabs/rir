...
RUN cd /opt/rir/external && (wget 10.200.14.25:8080/clang+llvm-11.0.0-x86_64-linux-gnu-ubuntu-20.04.tar.xz || true) && cd .. && tools/sync.sh && tools/build-gnur.sh custom-r && rm -rf external/custom-r/cache_recommended.tar .git && find external -type f -name '*.o' -exec rm -f {} \; &&\
